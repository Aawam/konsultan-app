...
Remaining manual smoke check: log in through the application once as
`admin@ypc.com` and once as `teknis@ypc.com` to confirm the browser session and
deployed frontend environment point to the same production project. No database
repair should be necessary if that check succeeds.
